...
function applyConfig(configRows: { configKey: string; configValue: string }[]): Chapter[] {
  ...
  return staticChapters.map((chapter) => {
    ...
    return {
      ...chapter,
      bgImage: updatedBg,
      questions: [...updatedQuestions, ...extras],
    };
  });
}
...
function GiveawayFormScreen({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!platform) return toast.error('請選擇參加平台');
    if (!socialHandle.trim()) return toast.error('請填寫帳戶名稱');
    if (!name.trim()) return toast.error('請填寫姓名');
    if (!email.trim()) return toast.error('請填寫電郵地址');
          ...
          <div data-default-content style={{ display: boardingPassImage ? 'none' : 'block' }}>
            <div className="px-6 pt-6 pb-4">
              ...
              <div className="flex gap-2 flex-wrap mb-4">
                {result.frequency.split(' · ').map((tag) => (
                  <span
                    ...
                    style={{ color: result.color, border: `1px solid ${result.color}50`, background: `${result.color}10` }}
                  >
                    {tag}
                  </span>
                ))}
              ...
              <p
                ...
                style={{ fontFamily: "'Noto Serif TC', serif", borderColor: result.color }}
              >
                "{result.tagline}"
              </p>
            </div>
            ...
            <div className="px-6 py-4">
              ...
              <div className="rounded-sm px-3 py-2.5" style={{ background: `${result.color}12`, border: `1px solid ${result.color}30` }}>
              ...
              <div className="mt-4 flex items-center gap-1 opacity-30">
                {Array.from({ length: 40 }).map((_, i) => (
                  <div
                    key={i}
                    className="bg-[#D4A843]"
                    style={{ width: i % 3 === 0 ? '3px' : '1.5px', height: i % 5 === 0 ? '20px' : '14px' }}
                  />
                ))}
              </div>
